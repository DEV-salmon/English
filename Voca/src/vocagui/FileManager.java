...
public class FileManager {
    ...
    /**
     * 사용자 ID를 기반으로 UserFileInfo(세션 객체)를 생성하고 반환합니다.
     * @param userId 로그인 성공한 사용자 ID
     * @return UserFileInfo 객체
     */
    public UserFileInfo prepareUserSession(String userId) {
        String baseDir = "Voca/src/res/Vocas";

        String userDirectory = baseDir + "/" + userId;;
        String vocaFile = userDirectory + "/" + "voca.txt";
        String incorrectFile = userDirectory + "/" + "incorrect.txt";
        String statFile = userDirectory + "/" + "stat.txt";

        return new UserFileInfo(userId, userDirectory, vocaFile, incorrectFile, statFile);
    }
}
